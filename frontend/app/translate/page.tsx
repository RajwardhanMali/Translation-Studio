...
function normalizeSegment(raw: Partial<Segment> & Record<string, unknown>, index: number): Segment {
  ...
  return {
    ...
    tm_suggestions: Array.isArray(raw.tm_suggestions) ? (raw.tm_suggestions as Segment['tm_suggestions']) : [],
    glossary_violations: Array.isArray(raw.glossary_violations) ? (raw.glossary_violations as string[]) : [],
    format_snapshot:
...
function isTranslationError(text: string | undefined) {
  return Boolean(text && (text.startsWith('[ERROR') || text.startsWith('[TRANSLATION ERROR')))
}
...
function StatusPill({ status }: { status: Segment['status'] | string | undefined }) {
  const map = {
    pending: {
      label: 'Pending',
      className:
...
function TmMatchBadge({ tmMatchType }: { tmMatchType: Segment['tm_match_type'] }) {
  ...
  return (
    <Badge variant="outline" className={cn('rounded-full px-2.5 text-[10px] font-semibold', map[tmMatchType])}>
      {tmMatchType === 'exact' ? 'TM exact' : tmMatchType === 'fuzzy' ? 'TM fuzzy' : 'New'}
    </Badge>
  )
}
...
interface SegmentRowProps {
  ...
  onApprove: (id: string, finalText: string) => Promise<void>
  onUpdate: (id: string, finalText: string) => void
  onRetry: (segment: Segment) => Promise<void>
  retrying: boolean
}
...
function SegmentRow({
  ...
  const [editValue, setEditValue] = useState(segment.final_text || segment.translated_text)
  const [approving, setApproving] = useState(false)
  const [showTM, setShowTM] = useState(false)

  useEffect(() => {
    setEditValue(segment.final_text || segment.translated_text)
  ...
  const handleApprove = async () => {
    setApproving(true)
    await onApprove(segment.segment_id, editValue)
    setEditing(false)
    setApproving(false)
  }
  ...
  const handleSave = () => {
    onUpdate(segment.segment_id, editValue)
    setEditing(false)
  }

  const isHeading = segment.type === 'heading'
  const isTableCell = segment.type === 'table_cell'
  const sourceTextClass = isHeading ? 'text-lg font-semibold leading-8' : 'text-sm leading-7'
  const outputTextClass = isHeading ? 'text-base font-semibold leading-8' : 'text-sm leading-7'
    ...
    >
      <div className="flex items-center justify-between gap-3 border-b border-border/60 px-4 py-3">
        ...
        <div className="flex items-center gap-2">
          {editing ? (
            <>
              <Button size="icon-sm" onClick={handleSave} className="rounded-xl">
                <Check className="h-3.5 w-3.5" />
              </Button>
              <Button size="icon-sm" variant="outline" onClick={handleCancel} className="rounded-xl">
            ...
            <>
              ...
              <Button size="icon-sm" variant="outline" onClick={handleEdit} className="rounded-xl">
      ...
      <div className="grid gap-4 p-4 xl:grid-cols-[1fr_1fr_1fr]">
        <div className="rounded-2xl border border-border/60 bg-background/65 p-4">
          <p className="mb-2 text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground">Source</p>
          <p className={cn('text-foreground', sourceTextClass)}>{segment.source_text}</p>
          {segment.glossary_violations && segment.glossary_violations.length > 0 && (
        ...
        <div className="rounded-2xl border border-border/60 bg-background/65 p-4">
          <div className="mb-2 flex items-center gap-2">
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground">Draft Translation</p>
          ...
          {isTranslationError(segment.translated_text) && (
            <div className="mt-4">
              <Button
                size="sm"
                variant="outline"
                className="rounded-xl border-red-300/70 text-red-700 hover:bg-red-50 hover:text-red-700 dark:border-red-800/70 dark:text-red-300 dark:hover:bg-red-950/40"
                onClick={() => void onRetry(segment)}
                disabled={retrying}
              >
                {retrying ? <Spinner className="mr-1.5 h-3.5 w-3.5" /> : <RotateCcw className="mr-1.5 h-3.5 w-3.5" />}
                Retry Segment
              </Button>
            </div>
          )}
          {(segment.tm_suggestions?.length ?? 0) > 0 && (
            ...
            >
              ...
              {segment.tm_suggestions!.length} suggestion{segment.tm_suggestions!.length !== 1 ? 's' : ''}
              <ChevronRight className={cn('h-3.5 w-3.5 transition-transform', showTM && 'rotate-90')} />
            </button>
          )}
        </div>

        <div className="rounded-2xl border border-border/60 bg-background/65 p-4">
          <p className="mb-2 text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground">Final Output</p>
          {editing ? (
            <Textarea
              value={editValue}
            ...
            <p
              className={cn(
                outputTextClass,
                segment.final_text ? 'text-foreground' : 'italic text-muted-foreground'
              )}
            >
              {segment.final_text || segment.translated_text || 'Not yet translated'}
            </p>
          )}
        </div>
      </div>

      {showTM && segment.tm_suggestions && segment.tm_suggestions.length > 0 && (
        <div className="border-t border-border/60 bg-muted/35 px-4 py-4">
          ...
          <div className="grid gap-3">
            {segment.tm_suggestions.map((tm, index) => (
              ...
              >
                <p className="flex-1 text-sm leading-7 text-foreground">{tm.text}</p>
                <div className="flex items-center gap-2">
                  <Badge
                    variant="outline"
                    className={cn(
                  ...
                  <Button
                    ...
                    className="rounded-xl"
                    onClick={() => {
                      onUpdate(segment.segment_id, tm.text)
                      setShowTM(false)
                    }}
                  >
                    Use Suggestion
...
function TranslatePageContent() {
  ...
  const loadSegments = async (nextStatusFilter: StatusFilter = statusFilter) => {
    setLoading(true)
    ...
    try {
      ...
      setSegments(
        data
          .map((segment, index) => normalizeSegment(segment as Partial<Segment> & Record<string, unknown>, index))
  ...
  const refreshSingleSegment = async (segmentId: string) => {
    const refreshed = (await getSegments(docId, getStatusParam(statusFilter)))
      .map((segment, index) => normalizeSegment(segment as Partial<Segment> & Record<string, unknown>, index))
      .filter(isVisibleSegment)
    const nextSegment = refreshed.find((segment) => segment.segment_id === segmentId)

    if (!nextSegment) {
      setSegments((prev) => prev.filter((segment) => segment.segment_id !== segmentId))
      return
    }

    setSegments((prev) => prev.map((segment) => (segment.segment_id === segmentId ? nextSegment : segment)))
  }
  ...
  const handleTranslate = async () => {
    ...
    try {
      ...
      setTranslationMessage(`${data.segments_translated} new segments translated`)
      toast({ title: 'Translation complete', description: `${data.segments_translated} new segments translated.` })
    } catch (error) {
      setTranslationMessage('Translation failed. Please try again.')
      toast({
        title: 'Translation failed',
        description: 'Could not run translation. Ensure the backend is running and the API URL is configured correctly.',
        variant: 'destructive',
      })
    } finally {
  ...
  const handleRetry = async (segment: Segment) => {
    ...
    try {
      ...
      const exportData = await getExportStatus(docId).catch(() => null)
      setExportStatus(exportData)
      toast({ title: 'Segment retried', description: 'The failed segment was sent back for translation.' })
    } catch {
      toast({
        title: 'Retry failed',
  ...
  const handleApprove = async (segmentId: string, finalText: string) => {
    try {
        ...
        setSegments((prev) =>
          prev.map((segment) =>
            segment.segment_id === segmentId
              ? { ...segment, status: response.status as Segment['status'], final_text: finalText }
              : segment
          )
        )
      } else {
      ...
      const exportData = await getExportStatus(docId).catch(() => null)
      setExportStatus(exportData)
      toast({ title: 'Segment approved', description: 'Correction saved and learning triggered.' })
    } catch {
      toast({
  ...
  const handleUpdate = (segmentId: string, finalText: string) => {
    setSegments((prev) =>
      prev.map((segment) => (segment.segment_id === segmentId ? { ...segment, final_text: finalText } : segment))
    )
  }
  ...
  const handleBulkApprove = async () => {
    for (const id of selectedIds) {
      ...
      if (segment) await handleApprove(id, segment.final_text || segment.translated_text)
    }
    setSelectedIds(new Set())
    toast({ title: `${selectedIds.size} segments approved` })
  }

  const completedCount = segments.filter((segment) => segment.status === 'reviewed' || segment.status === 'approved').length
  const approvedCount = segments.filter((segment) => segment.status === 'approved').length
  const pendingCount = segments.filter((segment) => segment.status === 'pending').length
  const flaggedCount = segments.filter(
    (segment) =>
      isTranslationError(segment.translated_text) || (segment.glossary_violations && segment.glossary_violations.length > 0)
  ).length
  const progress = segments.length ? Math.round((completedCount / segments.length) * 100) : 0
  ...
  return (
    <AppShell
      ...
      actions={
        <div className="flex items-center gap-2">
          {selectedIds.size > 0 && (
            <Button size="sm" variant="outline" className="rounded-2xl" onClick={() => void handleBulkApprove()}>
          ...
          >
            {exporting ? <Spinner className="mr-1.5 h-3.5 w-3.5" /> : <Download className="mr-1.5 h-3.5 w-3.5" />}
            Download
          </Button>
          <Button size="sm" variant="outline" className="rounded-2xl" onClick={() => router.push('/glossary')}>
    ...
    >
      <div className="space-y-6">
          ...
          <div className="grid gap-6 px-6 py-6 lg:grid-cols-[1.2fr_0.8fr] lg:px-8 lg:py-8">
            <div>
              ...
              <p className="mt-3 max-w-2xl text-sm leading-7 text-muted-foreground md:text-base">
                Generate drafts, inspect glossary risks, compare translation memory suggestions, and approve final copy
              ...
              <div className="mt-6 flex flex-col gap-3 md:flex-row md:flex-wrap md:items-center">
                ...
                <div className="flex items-center gap-3 rounded-2xl border border-border/60 bg-background/75 px-4 py-3">
                  <div className="min-w-[180px]">
                    ...
                    <Select value={exportFormat} onValueChange={(value) => setExportFormat(value as ExportFormat)}>
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="docx">Word document - better fidelity</SelectItem>
                  ...
                  <Button
                    ...
                    title={exportFormat === 'docx' ? 'Recommended for stronger layout fidelity.' : undefined}
                  >
                    {exporting ? <Spinner className="mr-2 h-4 w-4" /> : <Download className="mr-2 h-4 w-4" />}
                    Export
                  </Button>
                  {!exportStatus?.ready_to_export && exportStatus && (
                    <Button variant="ghost" className="h-11 rounded-2xl" onClick={() => void handleExport()} disabled={exporting}>
                      Export Anyway
                    </Button>
                  )}
                </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {[
                { label: 'Review progress', value: `${progress}%`, note: `${completedCount}/${segments.length} completed` },
                { label: 'Approved', value: `${approvedCount}`, note: 'Finalized and learned' },
                { label: 'Pending', value: `${pendingCount}`, note: 'Needs review' },
                { label: 'Attention needed', value: `${flaggedCount}`, note: 'Errors or glossary alerts' },
              ].map((item) => (
                <div
                  key={item.label}
          ...
          <Card className="glass-panel self-start rounded-[1.75rem] border-border/70 p-5">
            <div className="flex flex-col gap-4">
              <div>
                ...
                <p className="mt-2 text-sm leading-7 text-muted-foreground">
                  Filter the queue, keep an eye on progress, and process segments in focused batches.
                </p>
              </div>
              ...
              <div className="w-full rounded-2xl border border-border/60 bg-background/75 p-4 sm:max-w-sm">
                <div className="flex items-center justify-between text-xs font-medium text-muted-foreground">
                  ...
                  <span className="font-mono">{completedCount}/{segments.length}</span>
                </div>
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              {STATUS_FILTERS.map((filter) => {
                const count = filter === 'all' ? segments.length : segments.filter((segment) => segment.status === filter).length
                return (
                  <button
                    key={filter}
          ...
          <Card className="glass-panel self-start rounded-[1.75rem] border-border/70 p-5">
            ...
            <div className="mt-4 space-y-3">
              {(['pending', 'reviewed', 'approved'] as const).map((status) => (
                <div key={status} className="flex items-center justify-between rounded-2xl border border-border/60 bg-background/70 px-3 py-2">
            ...
            <div className="mt-5 rounded-2xl border border-border/60 bg-background/70 p-4">
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground">TM Match Types</p>
        ...
        {exportStatus && (exportStatus.pending > 0 || exportStatus.translation_errors > 0 || exportStatus.warning) && (
        ...
        {segments.some((segment) => segment.glossary_violations && segment.glossary_violations.length > 0) && (
          <div className="glass-panel flex items-center gap-3 rounded-[1.5rem] border border-amber-300/60 bg-amber-100/75 px-5 py-4 dark:border-amber-800/60 dark:bg-amber-950/35">
            ...
            <p className="text-sm text-amber-800 dark:text-amber-200">
              Some segments contain glossary term violations. They are highlighted inline for faster review.
            </p>
          </div>
        )}
        ...
        {loading ? (
          ...
          <div className="glass-panel flex flex-col items-center gap-3 rounded-[2rem] py-20 text-center">
            ...
            <p className="text-lg font-semibold text-foreground">No segments found</p>
            <p className="max-w-md text-sm leading-7 text-muted-foreground">
              Try another filter or run translation to populate this document with segments.
            </p>
          </div>
        ) : (
